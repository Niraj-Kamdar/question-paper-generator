...
class IsSumOf(object):
    ...
    def __call__(self, form, field):
        try:
            ...
            raise ValidationError(field.gettext("Invalid field name in {}.").format(', '.join(self.fieldnames)))
        if field.data != expected_sum:
            message = self.message
            if message is None:
                message = field.gettext('Field must be equal to {}.'.format(expected_sum))
...
class MarkDistributionForm:
    def __init__(self, course_id, questions, total_marks):
        ...
        data.update({"total_marks": IntegerField(total_marks,
        ...
        for constraint in self.fields:
            for field in self.fields[constraint]:
                ...
                self.flatten_data[constraint][self.translate(constraint, field)] = int(field_attr.data)
