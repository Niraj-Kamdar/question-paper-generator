from flask import render_template, flash, redirect, url_for, json, Blueprint, request
...
def question(course_id, unit_id, qtype):
    ...
    if qtype == "mcq":
        _mcq_questions = MCQQuestion.query.filter(MCQQuestion.unit_id == unit_id).paginate(page=main_page,
                                                                                           per_page=1)
...
@questions.route("/course/<course_id>/unit/<unit_id>/question/<qtype>/new/", methods=["GET", "POST"])
@login_required
@check_valid_course
@check_valid_unit
@check_valid_question_type
...
@questions.route("/course/<course_id>/unit/<unit_id>/question/<qtype>/update/<int:question_id>/",
                 methods=["GET", "POST"])
@login_required
@check_valid_course
@check_valid_unit
...
@questions.route("/course/<course_id>/unit/<unit_id>/question/<qtype>/imp/<impq>/", methods=["GET"])
@login_required
@check_valid_course
@check_valid_unit
@check_valid_question_type
