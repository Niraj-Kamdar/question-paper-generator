(function() {
{
  const sideNavigationContainer =
      document.getElementById("side_navigation_container"); // side nav

  // open side nav on click on bar button
  document.getElementById("bars_container")
  ...
  {
    const mainContainer = document.getElementById("main_container"); // main
                                                                     // page

    const initialLeft =
        parseInt(window.getComputedStyle(sideNavigationContainer)
    ...
    let mainFlag = false;
    let netDistance = 0;
    // mainContainer.style.paddingTop = "100px"; // for all pages except
    // question list

    mainContainer.addEventListener("touchstart", function(e) {
      let touchObj = {};
      if (window.innerWidth > 920)
        return; // do not process if screen width is greater than 920
      touchObj = e.changedTouches[0];
    ...
    mainContainer.addEventListener("touchmove", function(e) {
      ...
      if (mainTouchStartX > 100)
        return; // if user touched from screen at distance greater than 100px
...

{
  ...
  let isLogin = false;
  {
    ...
    isHomePage = !!(url.match(new RegExp("/", "g")).length === 3 &&
                    url[url.length - 1] === "/");
    isLogin = url.indexOf("login") !== -1 || url.indexOf("register") !== -1
                  ? true
                  : false;
  ...
  }
  {
    ...
    if (isHomePage || areAdditionalPages) {
      ...
      sideNavigationItems[1].style.display =
          sideNavigationItems[10].style.display = "none";
    } else if (isLogin) {
      sideNavigationItems[5].parentNode.style.display = "none";
      sideNavigationItems[1].style.display =
          sideNavigationItems[10].style.display =
              sideNavigationItems[9].style.display = "none";
    } else {
      sideNavigationItems[2].style.display =
          sideNavigationItems[3].style.display =
              sideNavigationItems[4].style.display =
                  sideNavigationItems[9].style.display = "none";
    }

    // disable links if href and window location is same for side navigation
    // links
...

  {
    ...
    if (isHomePage || areAdditionalPages) {
      ...
      topNavigationItems[1].style.display =
          topNavigationItems[9].style.display = "none";
    } else if (isLogin) {
      topNavigationItems[5].parentNode.parentNode.style.display = "none";
      topNavigationItems[1].style.display =
          topNavigationItems[9].style.display =
              topNavigationItems[8].style.display = "none";
    } else {
      topNavigationItems[2].style.display =
          topNavigationItems[3].style.display =
              topNavigationItems[8].style.display =
                  topNavigationItems[4].style.display = "none";
    }
    // disable links if href and window location is same for top navigation
    // links
      ...
      topNavigationItems[i].addEventListener("click", function(e) {
        if (
            ...
            e.target.parentNode.href + "/" ===
                window.location.href // for course/
        )
          e.preventDefault();
      });
    }
...

{
  ...
  dropdownTitle.addEventListener("touchmove", function(e) {
    let touchObj = {};
    touchObj = e.changedTouches[0];
    if (!(Math.abs(touchObj.pageX - startX) <= 10) ||
