(function(){
    ...
    const toggleContainer = Array.from(document.getElementsByClassName("toggle_container"));
    ...
    const qids = Array.from(document.getElementsByClassName("first_row_column_1")).map((node)=>Number(node.getAttribute("data-id")));
    const qtypes = Array.from(document.getElementsByClassName("first_row_column_3")).map((node)=>node.innerHTML);
    const logoForm = document.getElementById("logoform");
    const paperFields = Array.from(document.getElementsByClassName("paper_fields"));
    const formClientErrs = Array.from(document.getElementsByClassName("form_client_error"));
    ...
    toggleContainer.forEach(function(node,index){
     ...
     conflictsButton.addEventListener('click',function(){
         const mcqAskId = qids.filter((node,index)=>addCheckbox[index].checked && qtypes[index]==='mcq');
         const mcqNaskId = qids.filter((node,index)=>!addCheckbox[index].checked && qtypes[index]==='mcq');
         const subAskId = qids.filter((node,index)=>addCheckbox[index].checked && qtypes[index]==='sub');
         const subNaskId = qids.filter((node,index)=>!addCheckbox[index] && qtypes[index]==='sub');
         const data = {
             "mcq":{
                 "ask" : mcqAskId,
                 "nask" : mcqNaskId
         ...
         fetch("/papers/handle/conflicts",{
             ...
             if(status==='OK'){
                 ...
                 conflictsButton.style.display = "none";
             }
         }).catch((err)=>{
             console.log(err);
         });
     ...
     paperFields.forEach(function(node,index){
         node.addEventListener('input',function(){
            formClientErrs[index].innerHTML = "";
         });
    });
    ...
    logoForm.addEventListener('submit',function(e){
        ...
        if(!status.isValid){
            formClientErrs.forEach((node,index)=>{node.innerHTML = status.err[index]});
            e.preventDefault();
        }
    })
