(function () {
  ...
  const toggleContainer = Array.from(
    document.getElementsByClassName("toggle_container")
  );
  const toggleButton = Array.from(
    document.getElementsByClassName("toggle_btn")
  );
  const addCheckbox = Array.from(
    document.getElementsByClassName("add_checkbox")
  );
  const qidContainer = Array.from(
    document.getElementsByClassName("first_row_column_1")
  );
  const qids = qidContainer.map((node) => Number(node.getAttribute("data-id")));
  const qtypes = Array.from(
    document.getElementsByClassName("first_row_column_3")
  ).map((node) => node.innerHTML.trim());
  const logoForm = document.getElementById("logoform");
  const paperFields = Array.from(
    document.getElementsByClassName("paper_fields")
  );
  const formClientErrs = Array.from(
    document.getElementsByClassName("form_client_error")
  );
  const cognitiveSkills = Array.from(
  ...
  toggleContainer.forEach(function (node, index) {
    node.addEventListener("click", function () {
      if (addCheckbox[index].checked) {
        ...
        toggleContainer[index].style.backgroundColor =
          "rgba(144, 0, 240, 0.76)";
      }
  ...
  logoForm.addEventListener("submit", function (e) {
    ...
    if (!status.isValid) {
      formClientErrs.forEach((node, index) => {
        node.innerHTML = status.err[index];
      });
      e.preventDefault();
    }
  });
