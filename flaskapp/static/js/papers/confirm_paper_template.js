(function () {
  const totalQuestions = (Questions) =>
    Questions.map((obj) => Object.keys(obj))
      .map((subQuestion, index) =>
        subQuestion.map((key) => "Q" + (index + 1) + key.toUpperCase())
      )
  ...
  const generateContent = (properties, className) => {
    ...
    return properties
      .reduce((acc, cur) => {
        return acc.map(
          (val, index) =>
            val + `<span class='${className}'>` + cur[index] + "</span>"
        );
      }, ini)
      .map((val) => "<div class='question_properties'>" + val + "</div>");
  };

  const subjectiveMarkDistribution = document.getElementById(
    "subjective_mark_distribution"
  );
  ...
  subQuestions = subQuestions.map(
    (val) => '<div class="subquestions_id">' + val + "</div>"
  );
  ...
  subjectiveMarkDistribution.innerHTML = subQuestions
    .map((element, index) => element + subProperties[index])
    .reduce((acc, cur) => acc + cur);
  mcqMarkDistribution.innerHTML = mcqs
    .map((element, index) => element + mcqProperties[index])
    .reduce((acc, cur) => acc + cur);
  ...
  generatePaper.addEventListener("click", function () {
    ...
    fetch(`/course/${courseId}/papers/confirm/template/`, {
      ...
      .then((paper) => {
        ...
        document.getElementById("template_display").innerHTML = paper.substr(
          index1 + 1,
          index2 - index1 - 1 + 1 - 1
        );
