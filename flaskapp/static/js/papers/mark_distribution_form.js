...
const questionTypeFields =
    Array.from(document.getElementsByClassName("question_type"));
const unitFieldsErr = Array.from(document.getElementsByClassName("unit_err"));
const cognitiveFieldsErr =
    Array.from(document.getElementsByClassName("cognitive_err"));
const difficultyFieldsErr =
    Array.from(document.getElementsByClassName("difficulty_err"));
const questionTypeFieldsErr =
    Array.from(document.getElementsByClassName("question_type_err"));
const questionsFieldsErr =
    Array.from(document.getElementsByClassName("question_err"));
...
const formParts = [
  unitFields, cognitiveFields, difficultyFields, questionTypeFields,
  questionsFields
];
const formPartsErr = [
  unitFieldsErr, cognitiveFieldsErr, difficultyFieldsErr, questionTypeFieldsErr,
  questionsFieldsErr
];

hiddenLabel.forEach(function(node) { node.style.display = "none"; });
...
fieldHeader.forEach(function(node) { node.style.display = "none"; });
...
markForm.addEventListener("submit",
                          function(e) { navigationHandler(3, false, e); });
...
nextBtn.forEach(function(node, index) {
  if (index !== 4) {
    node.addEventListener('click',
                          function() { navigationHandler(index, false); });
  }
});

backBtn.forEach(function(node, index) {
  node.addEventListener('click',
                        function() { navigationHandler(index, true); });
});
...
function isValidTotalMarks(element) {
  const total_marks =
      element.map(node => Number(node.value)).reduce((acc, cur) => acc + cur);
