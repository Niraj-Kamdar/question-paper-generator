...
function deleteCoursesandUnits(deleteBtns, deleteBox, url) {
  ...
  deleteBtns.forEach(function(node) {
    node.addEventListener('click', function() {
      if (!isVisible) {
        isVisible = true;
        Array.from(deleteBox).forEach(
            (node) => { node.style.display = "initial"; });
      } else {
        isVisible = false;
        const selectedNodes =
            Array.from(deleteBox).filter((node) => node.checked);
        const ids = selectedNodes
                        .map((node) => {
                          return node.parentNode.parentNode.lastElementChild
...
deleteCourseBox && deleteCourseBtn &&
    deleteCoursesandUnits([ smallDeleteCourseBtn, deleteCourseBtn ],
                          deleteCourseBox, '/course/delete/');

deleteUnitBox && deleteUnitBtn &&
    deleteCoursesandUnits([ smallDeleteUnitBtn, deleteUnitBtn ], deleteUnitBox,
