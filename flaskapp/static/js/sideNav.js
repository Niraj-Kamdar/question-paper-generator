...
if (index1 === -1) {
  ...
  if (flashes.length) {
    // console.log("yup");
    setTimeout(() => { flashes[0].style.display = "none"; }, 1000);
  }
...
const coursesItemContainer =
    document.getElementsByClassName("courses_items_container");
...
for (let i = 0; i < courseName.length; i++) {
  ...
  if (cid === courseId[i].innerText) {
    coursesItemContainer[0].innerHTML =
        ...
        "<a class='course_link'>" + courseName[i].innerText + "</a>" +
        "</div>";
  } else {
    coursesItemContainer[0].innerHTML =
        coursesItemContainer[0].innerHTML + "<div class='courses_items'>" +
        "<img alt='list_style_image' src='/static/images/list_style_image.svg' /> " +
        "<a href='" + completeUrl + "' class='course_link'>" +
        courseName[i].innerText + "</a>" +
        "</div>";
  }
}
})();
