...
class impfunction(unittest.TestCase):
    ...
    def test_imp(self):
        ...
        self.assertEqual(response.status_code, 200)
        q1 = self.session.query(Question).filter_by(id=1).first()
        q2 = self.session.query(Question).filter_by(id=2).first()
        q3 = self.session.query(Question).filter_by(id=3).first()
