...
class HelpTestCase(BaseUser):
    def test_help(self):
        ...
        self.assertIn(b"Maybe there are not enough question to meet your requirement", response.data)
        self.assertIn(b"Maybe Too much questions are marked as ‘always include", response.data)
        self.assertIn(b"marked questions will be more than required", response.data)
