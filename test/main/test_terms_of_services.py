...
class TermsOfServiceTestCase(BaseUser):
    def test_terms_of_service(self):
        ...
        self.assertIn(b"responsible for compliance with any applicable local laws.", response.data)
        self.assertIn(b"The materials contained in this website are protected", response.data)
        ...
        self.assertIn(b"be included in all copies or substantial portion", response.data)
        self.assertIn(b"This license shall automatically terminate if you", response.data)
        self.assertIn(b"any of these restrictions and may be terminated by", response.data)
        self.assertIn(b"all other warranties including, without limitation,", response.data)
        self.assertIn(b"warranties or conditions of merchantability, fitnes", response.data)
        ...
        self.assertIn(b"SetNow may revise these terms of service for its website", response.data)
        self.assertIn(b"By using this website you are agreeing to be bound by the then", response.data)
        self.assertIn(b"These terms and conditions are governed by and construed in", response.data)
        self.assertIn(b"and you irrevocably submit to the exclusive jurisdiction of", response.data)
